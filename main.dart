 ...
 print("jokelength.first: ${jokelength.first}");
  ...
  for (var item in list) {
    ...
    temperatures.add(temp);
      ...
      double Temp_average = temperatures.reduce((a, b) => a + b) / temperatures.length;
  double error_percentage_resault_max_temp = 0;
  double error_percentage_resault_min_temp = 0;
  void error_percentage_temp() {
    error_percentage_resault_min_temp = ((temperatures.first - Temp_average).abs() / Temp_average) * 100;
     error_percentage_resault_max_temp = (temperatures.last - Temp_average).abs() / Temp_average * 100;
      print("the min error precentage of temps is $error_percentage_resault_min_temp");
      print("the max error precentage of temps is $error_percentage_resault_max_temp");
  }
  double error_percentage_resaultMinJoke = 0;
  double error_percentage_resault_max_joke = 0;
  void error_percentage_joke() {
    error_percentage_resaultMinJoke = (jokelength.first - jokeaverage).abs() / jokeaverage * 100;
    error_percentage_resault_max_joke = (jokelength.last - jokeaverage).abs() / jokeaverage * 100; 
      print("the max error precentage of jokes is $error_percentage_resault_max_joke%");
      print("the min error precenttage of jokes is $error_percentage_resaultMinJoke%");
  }
